...
from config.settings import (
    ...
    LOG_LEVEL, LOG_FILE, EXCEL_FILE_PATH
)
...
class LeadCollectionAgentExcel:
    """Главный агент для сбора лидов с Excel"""

    def __init__(self):
        """Инициализация агента и всех модулей"""
        logger.info("Инициализация агента с Excel...")
    ...
    def _load_system_prompt(self) -> str:
        ...
        return f"""ТЫ – ИНТЕЛЛЕКТУАЛЬНЫЙ АССИСТЕНТ ДЛЯ СБОРА ИНФОРМАЦИИ О КОМПАНИЯХ. ТЫ ОБЛАДАЕШЬ ПОЛНЫМ ДОСТУПОМ К НАБОРУ ИНСТРУМЕНТОВ ДЛЯ ПОИСКА, АНАЛИЗА И ФИКСАЦИИ ДАННЫХ В ЭКСЕЛЬ-ФАЙЛЫ. ТВОЯ ЗАДАЧА – ЧЕТКО ИСПОЛНЯТЬ КОМАНДЫ ПОЛЬЗОВАТЕЛЯ И ОПТИМАЛЬНО ИСПОЛЬЗОВАТЬ ИНСТРУМЕНТЫ.

### ДАННЫЕ ПОЛЬЗОВАТЕЛЯ ###
- **Excel файл**: {EXCEL_FILE_PATH}
...
- **Инструменты парсинга**: Claude AI, Google Maps API
- **Платформа автоматизации**: n8n workflow

### ДОСТУПНЫЕ ИНСТРУМЕНТЫ ###
1. **AgentLeadAddQuery** – СОЗДАЕТ И СОХРАНЯЕТ ЗАПРОСЫ В ЭКСЕЛЬ-ФАЙЛ, ЕСЛИ ПОЛЬЗОВАТЕЛЬ ПРОСИТ СОБРАТЬ ЗАПРОСЫ, ТЫ ДОЛЖЕН ВЫПОЛНИТЬ ЭТОТ ИНСТРУМЕНТ
...
5. **QuerySheets** – ВОЗВРАЩАЕТ ЗАПРОСЫ, КОТОРЫЕ ЕЩЁ НЕ ОБРАБАТЫВАЛИСЬ
6. **SiteCompanySheets** – ВОЗВРАЩАЕТ САЙТЫ КОМПАНИЙ, ПО КОТОРЫМ ЕЩЁ НЕ БЫЛО СБОРА ДАННЫХ
7. **AgentLeadMailGenerate** – СОЗДАЁТ ЧЕРНОВИК ПИСЬМА И СОХРАНЯЕТ ЕГО В ЭКСЕЛЬ
...
Ты должен использовать инструменты через function calling. Отвечай ЧЕТКО и КРАТКО."""
    ...
    def _get_tools_definition(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "name": "QuerySheets",
                "description": "Возвращает список запросов из Excel со статусом = 0 (необработанные)",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "limit": {
            ...
            },
            {
                ...
                "description": "Создаёт и сохраняет поисковые запросы в Excel (лист ЗАПРОСЫ). Генерирует несколько вариантов запросов для поиска компаний по заданной теме и городу.",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "request": {
            ...
            },
            {
                ...
                "description": "Возвращает список сайтов компаний из Excel со статусом = 0 (данные не собраны)",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "limit": {
            ...
            },
            {
                ...
                "description": "Собирает сайты компаний из Google Maps по созданным запросам и сохраняет в Excel (лист САЙТЫ)",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "max_queries": {
                        ...
                        "target_count": {
                            "type": "integer",
                            "description": "Целевое количество сайтов для сбора",
                            "default": 250
                        }
                    },
                    "required": []
            ...
            },
            {
                ...
                "description": "Собирает детальную информацию о компаниях с их сайтов и сохраняет в Excel (лист КОМПАНИИ)",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "max_sites": {
            ...
            },
            {
                ...
                "description": "Создаёт персонализированные черновики писем для собранных компаний и сохраняет в Excel (лист ПИСЬМА)",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "max_companies": {
    ...
    def _tool_query_sheets(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Получить необработанные запросы из Excel"""
        limit = params.get("limit", 10)
        queries = self.excel.get_queries_with_status(status=0)
...
"""
        ...
        try:
            ...
            # Сохраняем в Excel
            success = self.excel.add_queries(queries)
    ...
    def _tool_site_company_sheets(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        limit = params.get("limit", 10)
        sites = self.excel.get_sites_with_status(status=0)
    ...
    def _tool_add_site_company(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        target_count = params.get("target_count", 250)

        # Получаем необработанные запросы
        queries = self.excel.get_queries_with_status(status=0)[:max_queries]
        ...
        try:
            all_sites_df = self.excel.get_all_companies()
            existing_sites = set(str(site.get('Сайт', '')).lower().strip() for site in all_sites_df if site.get('Сайт'))
        except:
            existing_sites = set()
        ...
        for query in queries:
            ...
            for place in places:
                site_url = str(place.get('сайт', '')).lower().strip()
            ...
            self.excel.update_query_status(query_id, status=1)

        # Сохраняем сайты
        if all_sites:
            success = self.excel.add_sites(all_sites)
            return {
                "success": success,
                "count": len(all_sites),
                "message": f"✅ Собрано {len(all_sites)} сайтов компаний (без дублей)\n\n📊 Все сайты добавлены в Excel.\n\n❓ Собрать контактные данные с сайтов?"
            }
    ...
    def _tool_scrap_information(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        max_sites = params.get("max_sites", 50)

        # Получаем сайты со статусом 0
        sites = self.excel.get_sites_with_status(status=0)[:max_sites]
        ...
        for site in sites:
            ...
            try:
                ...
                self.excel.update_site_status(site.get('ID', ''), status=1)
    ...
    def _tool_generate_mail(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        max_companies = params.get("max_companies", 30)

        # Получаем компании из Excel
        try:
            companies = self.excel.get_all_companies(limit=max_companies)
        except Exception as e:
...
3. **Формат письма:**
   ...
   - Текст: 150-250 слов, структурированный
...
{{
  ...
  "текст": "Текст коммерческого предложения"
    ...
    def run_interactive(self):
        ...
        print("=" * 60)
        print("🤖 ИНТЕЛЛЕКТУАЛЬНЫЙ АГЕНТ ДЛЯ СБОРА ЛИДОВ (EXCEL)")
        print("=" * 60)
        print(f"\n📊 Excel файл: {EXCEL_FILE_PATH}")
